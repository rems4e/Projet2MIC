...
const std::string* TiXmlElement::Attribute( const std::string& name, long* i ) const
{
	const std::string* s = Attribute( name );
	if ( i )
	{
...
const std::string* TiXmlElement::Attribute( const std::string& name, unsigned long* i ) const
{
	const std::string* s = Attribute( name );
	if ( i )
	{
		if ( s ) {
			*i = static_cast<unsigned long>(atoll( s->c_str()) );
		}
		else {
			*i = 0;
		}
...
const char* TiXmlElement::Attribute( const char* name, unsigned long* i ) const
{
	...
	{
		if ( s ) {
			*i = static_cast<unsigned long>(atoll( s ));
		}
		else {
			*i = 0;
		}
...
{	
	char buf[64];
#if defined(TIXML_SNPRINTF)		
	TIXML_SNPRINTF( buf, sizeof(buf), "%lu", val );
#else
	sprintf( buf, "%lu", val );
#endif
	SetAttribute( name, buf );
