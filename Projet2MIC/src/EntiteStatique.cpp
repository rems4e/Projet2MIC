...
EntiteStatique::EntiteStatique(bool decoupagePerspective, Niveau *n, uindex_t index, ElementNiveau::elementNiveau_t cat) : ElementNiveau(decoupagePerspective, n, index, cat), _image(), _cadres(0), _nb(1) {
	...
	glm::vec2 dimensions(_image.dimensions());
	if(e->Attribute("l"))
	...
	if(this->decoupagePerspective()) {
		...
		_cadres[0] = Rectangle(origine, glm::vec2(LARGEUR_CASE, dimensions.y));
		for(index_t i = 1; i < _nb; ++i) {
			_cadres[i].definirDimensions(_cadres[0].dimensions());
			_cadres[i].definirOrigine(glm::vec2((_cadres[i - 1].origine() + _cadres[i - 1].dimensions()).x, 0));
...
void EntiteStatique::afficher(index_t deltaY, glm::vec2 const &decalage) const {
	if(deltaY >= _nb)
		return;
	_image.afficher(this->positionAffichage() - decalage + glm::vec2(deltaY * 64, 0), this->decoupagePerspective() ? _cadres[deltaY] : _cadres[0]);
}
